...
def plot(
    ...
    generator_path = curr_dir / '..' / 'trained_model' / f'generator_{zone}'
    generator.load_state_dict(torch.load(generator_path, map_location=torch.device('cpu')))
    discriminator_path = curr_dir / '..' / 'trained_model' / f'discriminator_{zone}'
...
def main(
    ...
    torch.save(generator.state_dict(), f'./torchsde/trained_model/generator_{zone}')
    torch.save(discriminator.state_dict(), f'./torchsde/trained_model/discriminator_{zone}')

    #_, _, test_dataloader = get_data(batch_size=batch_size, device=device)

    plot(num_plot_samples=num_plot_samples, plot_locs=plot_locs)
